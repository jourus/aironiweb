...
define ( "AIRO_SQL_GET_ARCIERI_DA_ABBINARE", "select COGNOME as 'Cognome', NOME as 'Nome', COMP as 'Compagnia', PIAZZUOLA as Piazzola, POS as Posizione, tessera as Tessera, CLASSE as Classe, CATEGORIA as Categoria from ISCRITTI where TESSERA not in (select id from IMMAGINI) order by PIAZZUOLA asc, POS asc");
...
define ( "AIRO_SQL_GET_ARCIERI_ABBINATI", "select COGNOME as 'Cognome', NOME as 'Nome', COMP as 'Compagnia', PIAZZUOLA as Piazzola, POS as Posizione, tessera as Tessera, CLASSE as Classe, CATEGORIA as Categoria, a.NomeFile as Foto, Stampato as Stampato  from IMMAGINI a inner join ISCRITTI b on b.tessera = a.ID where (Stampato = 0 or ? = 1);");
define ( "AIRO_SQL_GET_ARCIERI_ABBINATI_BY_ID", "select COGNOME as 'Cognome', NOME as 'Nome', COMP as 'Compagnia', PIAZZUOLA as Piazzola, POS as Posizione, tessera as Tessera, CLASSE as Classe, CATEGORIA as Categoria, a.NomeFile as Foto, Stampato as Stampato  from IMMAGINI a inner join ISCRITTI b on b.tessera = a.ID where tessera = ?;");
define ( "AIRO_SQL_UPDATE_ARCIERE_STAMPATO", "update IMMAGINI set Stampato = 1 where `ID` = ?;");
